...
document.addEventListener('DOMContentLoaded', () => {
  ...
  items.forEach(item => {
    ...
    const openItem = (open) => {
      ...
      btn.setAttribute('aria-expanded', String(open));
      // ensure smooth height transition if CSS uses [hidden] or max-height tricks
      if (open) {
        panel.hidden = false;
      } else {
        // keep panel visible for height animation if needed; CSS controls max-height
        panel.hidden = false;
      }
    };
    ...
    const toggle = () => {
      ...
      items.forEach(i => {
        ...
        if (p) p.hidden = false; // allow CSS max-height animation
      });
      // open this one (if desired)
      if (willOpen) openItem(true);
    };
